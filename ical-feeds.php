...
function icalfeeds_conf() {
	...
	echo '<p>'.__('You can also use a custom end date field with <b>enddatefield</b> and a custom day repeat field with <b>repeatfield</b>.', ICALFEEDS_TEXTDOMAIN).'</p>';
...
function icalfeeds_feed() {
    ...
    //Custom repeat field
    if (isset($_GET['repeatfield'])) {
        $post_repeat_field = $_GET['repeatfield'];
    } else {
        $post_repeat_field = 1;
    }
    ...
    while ( $posts->have_posts() ) {
	    ...
	    if ($post_date_field !== 'pubDate') {
            ...
            $repeat = (get_post_meta( get_the_ID(), $post_repeat_field, true));
